...
use dawnstore_core::{abstractions::{
    BackendGetObjectsFilter, DawnstoreBackend, ForeignKey, NewDawnStoreBackend, ObjectRelation,
    RawForeignKeyConstraint, RawSchema, ResourceCache,
...
// ── NewDawnStoreBackend trait impl ────────────────────────────────────────────

impl NewDawnStoreBackend for PostgresBackend {
    fn load_all_schemas(
        &self,
    ...
    ) -> impl std::future::Future<Output = Result<Vec<ReturnObject<serde_json::Value>>, DawnStoreError>> + Send
    {
        ...
        async move { self.get_impl(&get_filter).await }
    }
    ...
    ) -> impl std::future::Future<Output = Result<Option<ReturnObject<serde_json::Value>>, DawnStoreError>> + Send
    {
        ...
        async move {
            let results = self.get_impl(&get_filter).await?;
